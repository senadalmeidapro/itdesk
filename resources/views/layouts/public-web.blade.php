...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="min-h-screen bg-zinc-50 font-sans text-zinc-900 antialiased">
        <header class="sticky top-0 z-40 border-b border-white/10 bg-brand-950/80 backdrop-blur-md">
            <div class="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
                ...
                <nav class="hidden items-center gap-7 md:flex">
                    <a href="{{ route('home') }}" class="text-sm font-medium text-brand-100/80 transition hover:text-white" wire:navigate>{{ __('Accueil') }}</a>
                    <a href="{{ route('services') }}" class="text-sm font-medium text-brand-100/80 transition hover:text-white" wire:navigate>{{ __('Services') }}</a>
                    <a href="{{ route('contact') }}" class="text-sm font-medium text-brand-100/80 transition hover:text-white" wire:navigate>{{ __('Contact') }}</a>
                </nav>
        ...
        <footer class="bg-brand-950 text-brand-100">
            <div class="mx-auto grid max-w-7xl gap-10 px-6 py-16 md:grid-cols-3">
                <div class="space-y-4">
                ...
                <div class="space-y-3 text-sm">
                    ...
                    <ul class="space-y-2 text-brand-200/70">
                        <li><a href="{{ route('services') }}" class="transition hover:text-white" wire:navigate>{{ __('Intervention & maintenance') }}</a></li>
                        <li><a href="{{ route('services') }}" class="transition hover:text-white" wire:navigate>{{ __('Réseaux & connectivité') }}</a></li>
                        <li><a href="{{ route('services') }}" class="transition hover:text-white" wire:navigate>{{ __('Support & helpdesk') }}</a></li>
                        <li><a href="{{ route('services') }}" class="transition hover:text-white" wire:navigate>{{ __('Sauvegarde & sécurité') }}</a></li>
                    </ul>
                </div>

                <div class="space-y-3 text-sm">
                    <p class="font-semibold text-white">{{ __('Contact') }}</p>
                    <ul class="space-y-2 text-brand-200/70">
                        <li>
